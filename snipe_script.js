...
function fnInjectOverviewBar(){
	...
	fnInjectUnits();
	win.$('<tr><td colspan="3">Village cible:<input id="snipe_coord" value="'+defaultCoords+'" class="text-input inactive" size="7" onFocus="this.select()" /></td><td colspan="1">Heure d\'impact:<input id="arrival_time" size="25" class="text-input inactive" value="'+defaultDate+'" onFocus="this.select()" /></td><td><input type="button" value="Go" onClick="fnCalculateBackTime()" /></td></tr>').insertAfter(win.$('#menu_row2'));
...
function fnCalculateBackTime(){
				...
				if(win.$("#view_"+units[ii]).attr("checked")){
					...
					/* if(troop_count > 0){ */
						...
						if(launchTime.getTime() > serverTime.getTime()){
							isVisible = true;
							output.push([launchTime.getTime(),"Envoyer "+units[ii]+"(" + troop_count + ") de [coord]"+source+"[/coord] sur [coord]"+target+"[/coord] \340 "+launchTime.toString().replace(/(\d*:\d*:\d*)(.*)/i,"$1"),e]);
						}
					/* } */
				}
			}
	...
	if(output.length > 0){		
		...
		srcHTML += "<span style=\"color:red;\">Impossible d'arriver \340 temps</span>";
	}
